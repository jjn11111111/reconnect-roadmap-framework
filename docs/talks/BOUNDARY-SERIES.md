# Boundary talks — beyond polite discourse

**North Star still applies:** compose the map; don't live in the comments section. These talks **push** — they do not **punch**.

Core series (Talks 1–5) builds trust. Boundary talks spend that trust on harder pivots.

---

## Boundary talks (available)

| Talk | Script | Theme |
|------|--------|-------|
| **7 — The Gregorian Spell** *(Part 1)* | [talk-07-the-gregorian-spell-script.md](./talk-07-the-gregorian-spell-script.md) | 22/7 vs 28/13 · 13×28 calendar · BC/AD spell |
| **7B — The Prison Matrix** *(Part 2)* | [talk-07-part-2-the-prison-matrix-script.md](./talk-07-part-2-the-prison-matrix-script.md) | Compression vs transverse · A440 · Darwin/method · victor history |
| **8 — The Zodiac Sigil** | [talk-08-the-zodiac-sigil-script.md](./talk-08-the-zodiac-sigil-script.md) | 12-house matrix · Phiscus · aspects |
| **9 — Number Personalities** | [talk-09-number-personalities-script.md](./talk-09-number-personalities-script.md) | 0–9 cast · Fibonacci vs doubling · hologram core |
| **13 — There Are No Outsiders** | [talk-13-four-layers-one-beam-script.md](./talk-13-four-layers-one-beam-script.md) | One container · four layers · laser coherence |
| **14 — The Other Half** | [talk-14-the-other-half-script.md](./talk-14-the-other-half-script.md) | 3-part fractal · incarnation ray · awakening paradox |
| **15 — Seed and Womb** | [talk-15-seed-and-womb-script.md](./talk-15-seed-and-womb-script.md) | Comet seed · collapsing inward · capacitance trinity |
| **16 — The Original Mirror** | [talk-16-the-original-mirror-script.md](./talk-16-the-original-mirror-script.md) | Water dipole · H/O · mirror before light |
| **21 — Assimilation vs Integration** *(hinge)* | [talk-21-assimilation-vs-integration-script.md](./talk-21-assimilation-vs-integration-script.md) | 7×3 · melt vs beam · door to wonder |
| **22 — The Philosopher's Stone** *(magick arc)* | [talk-22-philosophers-stone-script.md](./talk-22-philosophers-stone-script.md) | 4-wave · stargate · frequency lock · reverse curse |

**References:** [../calendar-22-7-28-13.md](../calendar-22-7-28-13.md) · [../four-layer-laser.md](../four-layer-laser.md) · [../three-part-fractal.md](../three-part-fractal.md) · [../comet-inversion.md](../comet-inversion.md) · [../water-dipole-mirror.md](../water-dipole-mirror.md) · [../assimilation-vs-integration.md](../assimilation-vs-integration.md) · [../philosophers-stone-four-wave.md](../philosophers-stone-four-wave.md) · [../talk-21-seven-times-three.md](../talk-21-seven-times-three.md) · [../the-bridge.md](../the-bridge.md) · [../prison-matrix.md](../prison-matrix.md) · [../zodiac-sigil.md](../zodiac-sigil.md) · [../numerology-personalities.md](../numerology-personalities.md)

**Perform Talk 7 Part 1 before Part 2** — or five-minute recap.

---

## Planned boundary talks (not yet scripted)

| # | Working title | Grok / framework source |
|---|---------------|-------------------------|
| 9 | **Who Picks the Pivot** | BC/AD, CE, CIA "conspiracy theory" dispatch |
| 10 | **The Veil and the Download** | Multidimensional perception suppressed; sovereignty |
| 11 | **Institutional Arrogance** | Peer review, credential condescension |
| 12 | **Transport as Hypothesis** | Full Ψ(res) formula; calendar temporal lock |
| 17 *(spine)* | **The Open Membrane** | [membrane-consumption-death.md](../membrane-consumption-death.md) · **ethics-first** · reciprocity not predation |
| 23+ | **10-Mode-9 Universe · Soldom Mandate** | All possible consciousness · teased Talk 22 close |

Request scripts in order of what you're ready to **perform**, not what angries the room most.

---

## Host pre-brief (required)

1. Content may challenge religious calendar norms — not individuals.  
2. "Spell" = mathematical container metaphor unless labeled otherwise.  
3. Speaker will state **hypothesis** before motive claims.  
4. Q&A: see Talk 6 rules — map, don't defend shell as fact.

---

## Rehearsal order after core series

3 → 1 → 4 → **9** → 7 → 7B → 8 → **13** → **14** → **15** → **16** → **21** → **22** → 5 → 6

Talk 7 lands best when audience already knows: zero hinge, 142857, middle node.  
Talks **13 → 14 → 15 → 16** (cosmology quartet): reunion · other half · seed/womb · **water mirror**.  
Talks **21 → 22** (hinge + magick): assimilation vs integration · **philosopher's stone · frequency lock**.
